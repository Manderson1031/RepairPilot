...
export default function App(){
 ...
 if(screen==='equipment')return <SafeAreaView style={s.safe}><GearBackdrop/><TopBar title="EQUIPMENT" back="home"/><ScrollView contentContainerStyle={s.dashboardWrap}>
  ...
  {equipment.filter(e=>`${e.name} ${e.manufacturer||''} ${e.model||''}`.toLowerCase().includes(equipmentSearch.toLowerCase())).map(e=><TouchableOpacity key={e.id} style={s.equipmentCard} onPress={()=>openEquipment(e)}><View style={s.machineThumb}><MaterialCommunityIcons name={(e.category||'').toLowerCase().includes('small')?'lawn-mower':'tractor'} size={35} color="#ffb000"/></View><View style={s.equipmentInfo}><Text style={s.equipmentName}>{e.name}</Text><Text style={s.equipmentModel}>{[e.manufacturer,e.model].filter(Boolean).join(' ')||'Equipment'}</Text>{e.serial?<Text style={s.serialText}>SN: {e.serial}</Text>:null}<View style={s.categoryBadge}><Text style={s.categoryText}>{(e.category||'EQUIPMENT').toUpperCase()}</Text></View></View><MaterialCommunityIcons name="chevron-right" size={28} color="#ffb000"/></TouchableOpacity>)}
 ...
 return <SafeAreaView style={s.safe}><GearBackdrop/><TopBar title="AI DIAGNOSIS" back="equipment"/><KeyboardAvoidingView style={{flex:1}} behavior={Platform.OS==='ios'?'padding':undefined} keyboardVerticalOffset={8}>
  ...
  {!last?<View style={s.symptomGrid}>{['Engine will not start','Loss of power','Overheating','Abnormal noise','Leaks','Electrical issue'].map((item,i)=><TouchableOpacity key={item} style={[s.symptomChoice,symptom===item&&s.symptomChoiceActive]} onPress={()=>setSymptom(item)}><MaterialCommunityIcons name={(['engine-off-outline','speedometer-slow','thermometer-alert','waveform','water-outline','car-battery'] as any)[i]} size={20} color={symptom===item?'#15110a':'#ffb000'}/><Text style={[s.symptomChoiceText,symptom===item&&s.symptomChoiceTextActive]}>{item}</Text><MaterialCommunityIcons name={symptom===item?'checkbox-marked':'checkbox-blank-outline'} size={20} color={symptom===item?'#15110a':'#777d7f'}/></TouchableOpacity>)}</View>:null}
...
,
 topBar:{height:62,flexDirection:'row',alignItems:'center',justifyContent:'space-between',paddingHorizontal:10,borderBottomWidth:1,borderBottomColor:'rgba(255,176,0,.62)',backgroundColor:'rgba(7,9,10,.97)'},wordmarkRow:{flexDirection:'row',alignItems:'center',gap:7},wordmarkIcon:{width:34,height:34,borderRadius:17},topBrand:{color:'#f5f5f2',fontSize:20,fontFamily:Platform.select({ios:'AvenirNextCondensed-Heavy',android:'sans-serif-condensed'}),fontWeight:'900',fontStyle:'italic',letterSpacing:.6},topIcon:{color:'#e9e9e5',fontSize:23,minWidth:26,textAlign:'center'},topHit:{width:40,height:40,alignItems:'center',justifyContent:'center'},previewBar:{backgroundColor:'#ffb000',zIndex:20},previewBarTitle:{height:24,flexDirection:'row',alignItems:'center',justifyContent:'center',gap:5},previewBarText:{color:'#15110a',fontSize:8,fontWeight:'900',letterSpacing:.5},previewPicker:{position:'absolute',top:24,left:8,right:8,backgroundColor:'#111415',borderWidth:1,borderColor:'#ffb000',padding:5,flexDirection:'row',flexWrap:'wrap',gap:4,shadowColor:'#000',shadowOpacity:.8,shadowRadius:12,shadowOffset:{width:0,height:6}},previewChoice:{width:'49.3%',paddingVertical:10,paddingHorizontal:10,borderWidth:1,borderColor:'#3d4345',backgroundColor:'#191c1d'},previewChoiceActive:{backgroundColor:'#ffb000',borderColor:'#ffd05a'},previewChoiceText:{color:'#e8e9e6',fontSize:11,fontWeight:'800'},previewChoiceTextActive:{color:'#15110a'},bottomNav:{height:64,flexDirection:'row',borderTopWidth:1,borderTopColor:'#3f4446',backgroundColor:'rgba(7,9,10,.99)',paddingBottom:5},navItem:{flex:1,alignItems:'center',justifyContent:'center'},navIcon:{color:'#8c9295',fontSize:19},navLabel:{color:'#858b8d',fontSize:8,marginTop:3,fontWeight:'700'},navActive:{color:'#ffb000'},
 ...
 dashboardWrap:{paddingHorizontal:12,paddingTop:10,paddingBottom:78},welcomeBlock:{paddingHorizontal:2,paddingBottom:9,borderBottomWidth:1,borderBottomColor:'rgba(255,176,0,.34)'},welcomeSmall:{color:'#d6d7d5',fontSize:11},welcomeName:{color:'#fff',fontSize:24,fontFamily:Platform.select({ios:'AvenirNextCondensed-Heavy',android:'sans-serif-condensed'}),fontWeight:'900'},welcomeSub:{color:'#9ba0a2',fontSize:11,marginTop:0},sectionHeader:{flexDirection:'row',alignItems:'center',gap:8,marginTop:11,marginBottom:7},sectionTitle:{color:'#ffb000',fontSize:13,fontFamily:Platform.select({ios:'AvenirNextCondensed-Heavy',android:'sans-serif-condensed'}),fontWeight:'900',letterSpacing:.5},sectionRail:{height:2,flex:1,backgroundColor:'rgba(255,176,0,.27)',borderRightWidth:24,borderRightColor:'rgba(112,116,116,.24)'},featureGrid:{flexDirection:'row',flexWrap:'wrap',gap:6},featureTile:{width:'32.15%',minHeight:104,borderWidth:1,borderColor:'rgba(255,176,0,.55)',borderRadius:4,backgroundColor:'rgba(12,14,15,.98)',paddingHorizontal:4,paddingVertical:8,alignItems:'center',justifyContent:'center',position:'relative',shadowColor:'#ffb000',shadowOpacity:.09,shadowRadius:6},featureIconBox:{width:35,height:31,alignItems:'center',justifyContent:'center',marginBottom:3},featureIconText:{color:'#ffb300',fontSize:27,fontWeight:'700'},featureTitle:{color:'#f5f5f2',fontFamily:Platform.select({ios:'AvenirNextCondensed-DemiBold',android:'sans-serif-condensed'}),fontWeight:'900',fontSize:10,textAlign:'center',letterSpacing:.1},featureSub:{color:'#9ea3a5',fontSize:8,lineHeight:11,textAlign:'center',marginTop:3},newBadge:{position:'absolute',right:4,top:4,backgroundColor:'#ffb000',color:'#111',fontWeight:'900',fontSize:7,paddingHorizontal:4,paddingVertical:2,borderRadius:2},activityRow:{borderWidth:1,borderColor:'#555b5d',borderRadius:4,backgroundColor:'rgba(14,16,17,.98)',paddingHorizontal:12,paddingVertical:10,flexDirection:'row',justifyContent:'space-between',alignItems:'center'},activityTitle:{color:'#f4f4f0',fontFamily:Platform.select({ios:'AvenirNextCondensed-DemiBold',android:'sans-serif-condensed'}),fontSize:15,fontWeight:'900'},chevron:{color:'#ffb000',fontSize:26,marginLeft:8},logoutLink:{padding:14,alignItems:'center'},
 searchRow:{flexDirection:'row',gap:8},searchInput:{flex:1,backgroundColor:'rgba(16,18,19,.98)',borderWidth:1,borderColor:'#565b5e',borderRadius:4,paddingHorizontal:12,paddingVertical:9,color:'#fff'},filterButton:{width:44,borderWidth:1,borderColor:'#ffb000',borderRadius:4,alignItems:'center',justifyContent:'center'},filterText:{color:'#ffb000',fontSize:26},addPanel:{borderWidth:1,borderColor:'#3b4144',backgroundColor:'rgba(12,15,16,.95)',borderRadius:5,padding:12,marginTop:10},listMeta:{flexDirection:'row',justifyContent:'space-between',marginTop:9,marginBottom:6,paddingHorizontal:2},metaText:{color:'#8c9295',fontSize:9,fontWeight:'800',letterSpacing:.4},equipmentRow:{flexDirection:'row',alignItems:'center',gap:10,borderBottomWidth:1,borderBottomColor:'#303537',paddingVertical:12},equipmentCard:{flexDirection:'row',alignItems:'center',gap:10,borderWidth:1,borderColor:'#4a4f51',borderLeftWidth:2,borderLeftColor:'#a97600',backgroundColor:'rgba(13,15,16,.99)',borderRadius:4,padding:8,marginBottom:6},equipmentInfo:{flex:1},equipmentName:{color:'#f5f5f2',fontFamily:Platform.select({ios:'AvenirNextCondensed-DemiBold',android:'sans-serif-condensed'}),fontWeight:'900',fontSize:15},equipmentModel:{color:'#a6abad',fontSize:10,marginTop:1},equipmentChevron:{color:'#ffb300',fontSize:28,paddingLeft:6},machineThumb:{width:62,height:52,borderRadius:3,borderWidth:1,borderColor:'#4c5254',backgroundColor:'#1b1f20',alignItems:'center',justifyContent:'center'},machineIcon:{fontSize:28,color:'#9da2a4'},serialText:{color:'#747b7e',fontSize:8,marginTop:3},categoryBadge:{alignSelf:'flex-start',borderWidth:1,borderColor:'#a97600',backgroundColor:'rgba(44,34,8,.76)',paddingHorizontal:6,paddingVertical:1,borderRadius:2,marginTop:4,maxWidth:110},categoryText:{color:'#ffc027',fontSize:7,fontWeight:'900'},selectedMachine:{flexDirection:'row',gap:10,alignItems:'center',borderWidth:1,borderColor:'#454b4d',borderTopColor:'#a97600',backgroundColor:'rgba(13,16,17,.98)',padding:10,marginBottom:12},stepper:{flexDirection:'row',alignItems:'center',paddingHorizontal:16},step:{width:28,height:28,borderRadius:14,borderWidth:1,borderColor:'#5c6264',alignItems:'center',justifyContent:'center',backgroundColor:'#111415'},stepActive:{width:28,height:28,borderRadius:14,alignItems:'center',justifyContent:'center',backgroundColor:'#ffb000'},stepNumber:{color:'#ddd',fontSize:12},stepNumberActive:{color:'#111',fontSize:12,fontWeight:'900'},stepLine:{height:1,backgroundColor:'#51575a',flex:1},stepLabels:{flexDirection:'row',justifyContent:'space-between',paddingHorizontal:2,marginTop:5,marginBottom:10},stepLabel:{color:'#8c9295',fontSize:9,width:'25%',textAlign:'center'},stepLabelActive:{color:'#ffb000',fontSize:9,fontWeight:'900',width:'25%',textAlign:'center'},diagnosisKicker:{color:'#ffb000',fontFamily:Platform.select({ios:'AvenirNextCondensed-Heavy',android:'sans-serif-condensed'}),fontWeight:'900',fontSize:14,letterSpacing:.5,marginBottom:2},diagnosisHint:{color:'#8f9698',fontSize:10,marginBottom:8},symptomGrid:{borderWidth:1,borderColor:'#3e4446',borderRadius:4,overflow:'hidden',marginBottom:8},symptomChoice:{height:42,flexDirection:'row',alignItems:'center',gap:9,paddingHorizontal:10,backgroundColor:'rgba(13,16,17,.98)',borderBottomWidth:1,borderBottomColor:'#303537'},symptomChoiceActive:{backgroundColor:'#ffb000'},symptomChoiceText:{flex:1,color:'#e8e9e6',fontSize:12,fontWeight:'700'},symptomChoiceTextActive:{color:'#15110a',fontWeight:'900'},attachmentRow:{flexDirection:'row',gap:6,marginBottom:8},attachmentButton:{flex:1,minHeight:52,borderWidth:1,borderColor:'#555b5d',borderRadius:4,backgroundColor:'rgba(18,20,21,.98)',alignItems:'center',justifyContent:'center',padding:5},attachmentText:{color:'#e5e6e3',fontSize:8,fontWeight:'800',marginTop:2,textAlign:'center'},
