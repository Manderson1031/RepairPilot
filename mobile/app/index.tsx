...
export default function App(){
 ...
 const TopBar=({title,back}:{title?:string,back?:Screen})=><View style={s.topBar}>{back?<TouchableOpacity onPress={()=>nav(back)}><Text style={s.topIcon}>‹</Text></TouchableOpacity>:<Text style={s.topIcon}>☰</Text>}<Text style={s.topBrand}>{title||<>REPAIR<Text style={s.green}>PILOT</Text></>}</Text><Text style={s.topIcon}>⚙</Text></View>;
 ...
 const FeatureTile=({icon,title,sub,onPress,badge}:{icon:string,title:string,sub:string,onPress:()=>void,badge?:string})=><TouchableOpacity style={s.featureTile} onPress={onPress}><View style={s.featureIcon}><Text style={s.featureIconText}>{icon}</Text></View>{badge?<Text style={s.newBadge}>{badge}</Text>:null}<Text style={s.featureTitle}>{title}</Text><Text style={s.featureSub}>{sub}</Text></TouchableOpacity>;
 const GearBackdrop=()=> <ImageBackground  source={require('../assets/industrial-bg.png')} resizeMode="cover" style={s.gearLayer} imageStyle={s.bgImage}><View style={s.bgShade}/><View style={s.steelTop}/><View style={s.hazardRail}/><View style={[s.bolt,{top:22,left:18}]}/><View style={[s.bolt,{top:22,right:18}]}/><View style={s.meshLine}/></ImageBackground>;
 ...
 if(!token || screen==='auth')return <SafeAreaView style={s.safe}><StatusBar style="light"/><GearBackdrop/><ScrollView contentContainerStyle={s.authWrap}>
  ...
  <TouchableOpacity style={s.createAccount} onPress={()=>auth(true)}><Text style={s.createAccountText}>CREATE ACCOUNT</Text></TouchableOpacity><Text style={s.version}>RepairPilot v0.19 beta</Text>
 </ScrollView></SafeAreaView>;
 ...
 if(screen==='home')return <SafeAreaView style={s.safe}><StatusBar style="light"/><GearBackdrop/><TopBar/><ScrollView contentContainerStyle={s.dashboardWrap}>
  <Text style={s.welcomeSmall}>Welcome back,</Text><Text style={s.welcomeName}>BRYAN</Text><Text style={s.welcomeSub}>Ready to diagnose and repair.</Text>
  <Text style={s.sectionTitle}>QUICK ACTIONS</Text><View style={s.featureGrid}>
  ...
  <Text style={s.sectionTitle}>CONTINUE WORKING</Text><TouchableOpacity style={s.activityRow} onPress={resumeDraft}><View><Text style={s.activityTitle}>Unfinished diagnosis</Text><Text style={s.featureSub}>Resume where you left off</Text></View><Text style={s.chevron}>›</Text></TouchableOpacity>
 ...
 if(screen==='maintenance')return <SafeAreaView style={s.safe}><StatusBar style="light"/><GearBackdrop/><TopBar title="MAINTENANCE" back="home"/><ScrollView contentContainerStyle={s.dashboardWrap}>
  <Text style={s.sectionTitle}>MAINTENANCE REMINDERS</Text><View style={s.emptyIndustrial}><Text style={s.heroToolIcon}>⚒</Text><Text style={s.activityTitle}>No maintenance reminders yet</Text><Text style={s.featureSub}>Maintenance schedules will be tied to your real equipment profiles—no sample machines or fake due dates.</Text></View>
 </ScrollView><BottomNav active="home"/></SafeAreaView>;
 ...
 if(screen==='equipment')return <SafeAreaView style={s.safe}><GearBackdrop/><TopBar title="EQUIPMENT" back="home"/><ScrollView contentContainerStyle={s.dashboardWrap}>
  ...
  <View style={s.listMeta}><Text style={s.featureSub}>{equipment.filter(e=>`${e.name} ${e.manufacturer||''} ${e.model||''}`.toLowerCase().includes(equipmentSearch.toLowerCase())).length} ITEMS</Text><Text style={s.featureSub}>SORT: NAME</Text></View>
  {equipment.filter(e=>`${e.name} ${e.manufacturer||''} ${e.model||''}`.toLowerCase().includes(equipmentSearch.toLowerCase())).map(e=><TouchableOpacity key={e.id} style={s.equipmentRow} onPress={()=>openEquipment(e)}><View style={s.machineThumb}><Text style={s.machineIcon}>⚙</Text></View><View style={{flex:1}}><Text style={s.activityTitle}>{e.name}</Text><Text style={s.featureSub}>{[e.manufacturer,e.model].filter(Boolean).join(' ')||'Equipment profile'}</Text><Text style={s.serialText}>{e.serial?`S/N ${e.serial}`:e.category}</Text></View><View style={s.categoryBadge}><Text style={s.categoryText}>{(e.category||'EQUIPMENT').toUpperCase()}</Text></View><Text style={s.chevron}>›</Text></TouchableOpacity>)}
...
,
 topBar:{height:58,flexDirection:'row',alignItems:'center',justifyContent:'space-between',paddingHorizontal:18,borderBottomWidth:1,borderBottomColor:'rgba(255,179,0,.35)',backgroundColor:'rgba(8,10,11,.94)'},topBrand:{color:'#f5f5f2',fontSize:21,fontWeight:'900',fontStyle:'italic',letterSpacing:.4},topIcon:{color:'#e9e9e5',fontSize:24,minWidth:28,textAlign:'center'},bottomNav:{height:66,flexDirection:'row',borderTopWidth:1,borderTopColor:'#33383a',backgroundColor:'rgba(8,10,11,.98)',paddingBottom:5},navItem:{flex:1,alignItems:'center',justifyContent:'center'},navIcon:{color:'#8c9295',fontSize:19},navLabel:{color:'#8c9295',fontSize:9,marginTop:3},navActive:{color:'#ffb300'},
 ...
 dashboardWrap:{padding:18,paddingBottom:86},welcomeSmall:{color:'#ddd',fontSize:13},welcomeName:{color:'#fff',fontSize:25,fontWeight:'900'},welcomeSub:{color:'#9ba0a2',fontSize:13,marginBottom:20},sectionTitle:{color:'#ffb300',fontSize:15,fontWeight:'900',marginTop:10,marginBottom:9},featureGrid:{flexDirection:'row',flexWrap:'wrap',gap:8},featureTile:{width:'48.7%',minHeight:138,borderWidth:1,borderColor:'rgba(255,179,0,.58)',borderRadius:7,backgroundColor:'rgba(15,18,19,.92)',padding:12,alignItems:'center',justifyContent:'center',position:'relative'},featureIcon:{height:42,justifyContent:'center'},featureIconText:{color:'#ffb300',fontSize:31,fontWeight:'700'},featureTitle:{color:'#f5f5f2',fontWeight:'900',fontSize:13,textAlign:'center'},featureSub:{color:'#a3a8aa',fontSize:11,lineHeight:15,textAlign:'center',marginTop:4},newBadge:{position:'absolute',right:6,top:6,backgroundColor:'#ffb300',color:'#111',fontWeight:'900',fontSize:8,paddingHorizontal:5,paddingVertical:2,borderRadius:3},activityRow:{borderWidth:1,borderColor:'#343a3d',borderRadius:7,backgroundColor:'rgba(16,19,20,.94)',padding:13,flexDirection:'row',justifyContent:'space-between',alignItems:'center'},activityTitle:{color:'#f4f4f0',fontSize:15,fontWeight:'900'},chevron:{color:'#ffb300',fontSize:26,marginLeft:8},logoutLink:{padding:18,alignItems:'center'},
 searchRow:{flexDirection:'row',gap:8},searchInput:{flex:1,backgroundColor:'rgba(17,20,21,.96)',borderWidth:1,borderColor:'#3b4144',borderRadius:7,padding:12,color:'#fff'},filterButton:{width:48,borderWidth:1,borderColor:'#ffb300',borderRadius:7,alignItems:'center',justifyContent:'center'},filterText:{color:'#ffb300',fontSize:26},addPanel:{borderWidth:1,borderColor:'#3b4144',backgroundColor:'rgba(12,15,16,.95)',borderRadius:8,padding:12,marginTop:12},listMeta:{flexDirection:'row',justifyContent:'space-between',marginVertical:14},equipmentRow:{flexDirection:'row',alignItems:'center',gap:10,borderBottomWidth:1,borderBottomColor:'#303537',paddingVertical:12},machineThumb:{width:52,height:52,borderRadius:5,borderWidth:1,borderColor:'#5c6264',backgroundColor:'#202425',alignItems:'center',justifyContent:'center'},machineIcon:{fontSize:27,color:'#ffb300'},serialText:{color:'#7f8689',fontSize:10,marginTop:3},categoryBadge:{borderWidth:1,borderColor:'#7b5c0a',backgroundColor:'#2a240f',paddingHorizontal:6,paddingVertical:3,borderRadius:3,maxWidth:76},categoryText:{color:'#ffbf22',fontSize:7,fontWeight:'900'},selectedMachine:{flexDirection:'row',gap:12,alignItems:'center',borderWidth:1,borderColor:'#343a3d',backgroundColor:'rgba(15,18,19,.95)',padding:12,marginBottom:15},stepper:{flexDirection:'row',alignItems:'center',paddingHorizontal:16},step:{width:28,height:28,borderRadius:14,borderWidth:1,borderColor:'#5c6264',alignItems:'center',justifyContent:'center',backgroundColor:'#111415'},stepActive:{width:28,height:28,borderRadius:14,alignItems:'center',justifyContent:'center',backgroundColor:'#ffb300'},stepNumber:{color:'#ddd',fontSize:12},stepNumberActive:{color:'#111',fontSize:12,fontWeight:'900'},stepLine:{height:1,backgroundColor:'#51575a',flex:1},stepLabels:{flexDirection:'row',justifyContent:'space-between',paddingHorizontal:2,marginTop:5,marginBottom:14},stepLabel:{color:'#8c9295',fontSize:9,width:'25%',textAlign:'center'},stepLabelActive:{color:'#ffb300',fontSize:9,fontWeight:'900',width:'25%',textAlign:'center'},
