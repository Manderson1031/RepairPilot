...
export default function ARAssistRoute(){
  ...
  useEffect(()=>{
    if(!running||!anchor||targeting)return;
    const timer=setInterval(async()=>{
      ...
      try{
        const [frame,projection]=await Promise.all([
          ...
          projectARAnchor(anchor.id,VIEW_W,VIEW_H)
        ]);
        setSnapshot(frame);
  ...
  const place=async(event:any)=>{
    ...
    const point={x:Math.max(0,Math.min(1,locationX/VIEW_W)),y:Math.max(0,Math.min(1,locationY/VIEW_H))};
    setBusy(true);
    try{
      const placed=await placeDepthARAnchor(point);
      const projection=await projectARAnchor(placed.id,VIEW_W,VIEW_H);
