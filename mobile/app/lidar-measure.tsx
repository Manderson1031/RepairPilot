...
export default function LidarMeasureRoute(){
  ...
  return <SafeAreaView style={s.safe}><StatusBar style="light"/><ImageBackground source={require('../assets/industrial-bg-v34.png')} style={s.bg} resizeMode="cover" imageStyle={{opacity:.82}}>
    ...
    <View style={s.wrap}>
      ...
      {measurement?<View style={s.result}><Text style={s.resultKicker}>{DIMENSIONS.find(([key])=>key===dimension)?.[1]} • 3D LiDAR DISTANCE</Text><Text style={s.distance}>{measurement.distance_mm.toFixed(2)} mm</Text><Text style={[s.confidence,measurement.confidence<.75&&s.confidenceLow]}>DEPTH CONFIDENCE {Math.round(measurement.confidence*100)}%</Text><Text style={s.depth}>Point depths: {(measurement.depth_m.start*1000).toFixed(0)} mm / {(measurement.depth_m.end*1000).toFixed(0)} mm</Text>{measurement.confidence<.75?<Text style={s.warning}>Below RepairPilot's 75% fusion threshold. This measurement will not be applied to exact hardware sizing.</Text>:<TouchableOpacity disabled={busy} style={s.apply} onPress={applyMeasurement}><MaterialCommunityIcons name="check-decagram" size={21} color="#111"/><Text style={s.applyText}>APPLY {DIMENSIONS.find(([key])=>key===dimension)?.[1]} TO HARDWARE SCAN</Text></TouchableOpacity>}</View>:null}
