...
export default function LidarMeasureRoute(){
  ...
  const [samples,setSamples]=useState<LidarPointMeasurement[]>([]);
  ...
  const series=useMemo(()=>{
    ...
    const distances=samples.map(sample=>sample.distance_mm);
    const value=median(distances);
    const spread=Math.max(...distances)-Math.min(...distances);
    const tolerance=Math.max(2,value*.03);
    const confidence=Math.min(...samples.map(sample=>sample.confidence));
    return {value,spread,tolerance,confidence,repeatable:spread<=tolerance&&confidence>=MIN_DEPTH_CONFIDENCE};
  },[samples]);
  ...
  const capture=async()=>{
    ...
    setBusy(true);resetSeries();setFrameSize({width:0,height:0});
    try{
  ...
  const choosePoint=async(event:any)=>{
    if(!snapshot||busy||frameSize.width<=0||frameSize.height<=0||samples.length>=REQUIRED_SAMPLES)return;
    ...
    if(next.length===2){
      ...
      try{
        ...
        setSamples(current=>[...current,measured].slice(0,REQUIRED_SAMPLES));
  ...
  const applyMeasurement=async()=>{
    if(!series){
      return Alert.alert('Repeat measurement',`RepairPilot requires ${REQUIRED_SAMPLES} LiDAR measurements of the same dimension before applying it to hardware sizing.`);
    }
    if(!series.repeatable){
      return Alert.alert('Measurement series not repeatable',`The three readings must each have at least 75% depth confidence and agree within ${series.tolerance.toFixed(1)} mm. Current spread is ${series.spread.toFixed(2)} mm. Recapture closer and repeat the series.`);
    }
    const payload={
      source:'arkit_lidar',
      confidence:series.confidence,
      measurements:{[dimension]:series.value},
      sample_count:REQUIRED_SAMPLES,
      repeatability_mm:series.spread
    };
  ...
  const sampleNumber=Math.min(samples.length+(points.length===2?0:1),REQUIRED_SAMPLES);
  ...
  return <SafeAreaView style={s.safe}><StatusBar style="light"/><ImageBackground source={require('../assets/industrial-bg-v34.png')} style={s.bg} resizeMode="cover" imageStyle={{opacity:.82}}>
    ...
    <View style={s.wrap}>
      ...
      <View style={s.instructions}><Text style={s.kicker}>WHAT ARE YOU MEASURING?</Text><View style={s.dimensionRow}>{DIMENSIONS.map(([key,label])=><TouchableOpacity key={key} disabled={busy} onPress={()=>{setDimension(key);resetSeries()}} style={[s.dimensionButton,dimension===key&&s.dimensionButtonActive]}><Text style={[s.dimensionText,dimension===key&&s.dimensionTextActive]}>{label}</Text></TouchableOpacity>)}</View><Text style={s.body}>Capture one depth frame, then measure the same two physical endpoints three times. RepairPilot uses the median reading and rejects the dimension when the three LiDAR measurements do not agree closely enough.</Text></View>
      <View style={s.seriesStatus}><Text style={s.seriesTitle}>{series?'REPEATABILITY CHECK':`SAMPLE ${sampleNumber} OF ${REQUIRED_SAMPLES}`}</Text><Text style={s.seriesText}>{series?`${series.value.toFixed(2)} mm median • ${series.spread.toFixed(2)} mm spread • allowed ${series.tolerance.toFixed(1)} mm`:samples.length?`${samples.length} measurement${samples.length===1?'':'s'} recorded. Reset the points and repeat the same endpoints.`:'Tap the first endpoint, then the second endpoint.'}</Text></View>
      ...
      {measurement&&!series?<View style={s.result}><Text style={s.resultKicker}>{selectedLabel} • SAMPLE {samples.length} OF {REQUIRED_SAMPLES}</Text><Text style={s.distance}>{measurement.distance_mm.toFixed(2)} mm</Text><Text style={[s.confidence,measurement.confidence<MIN_DEPTH_CONFIDENCE&&s.confidenceLow]}>DEPTH CONFIDENCE {Math.round(measurement.confidence*100)}%</Text><Text style={s.depth}>Point depths: {(measurement.depth_m.start*1000).toFixed(0)} mm / {(measurement.depth_m.end*1000).toFixed(0)} mm</Text>{samples.length<REQUIRED_SAMPLES?<TouchableOpacity disabled={busy} style={s.apply} onPress={nextSample}><MaterialCommunityIcons name="target" size={21} color="#111"/><Text style={s.applyText}>MEASURE SAME ENDPOINTS AGAIN</Text></TouchableOpacity>:null}</View>:null}
      {series?<View style={s.result}><Text style={s.resultKicker}>{selectedLabel} • 3-SAMPLE MEDIAN</Text><Text style={s.distance}>{series.value.toFixed(2)} mm</Text><Text style={[s.confidence,!series.repeatable&&s.confidenceLow]}>{series.repeatable?'REPEATABILITY PASSED':'REPEATABILITY FAILED'} • {Math.round(series.confidence*100)}% MIN CONFIDENCE</Text><Text style={s.depth}>Readings: {samples.map(sample=>sample.distance_mm.toFixed(2)).join(' / ')} mm</Text><Text style={s.depth}>Spread {series.spread.toFixed(2)} mm • maximum allowed {series.tolerance.toFixed(1)} mm</Text>{series.repeatable?<TouchableOpacity disabled={busy} style={s.apply} onPress={applyMeasurement}><MaterialCommunityIcons name="check-decagram" size={21} color="#111"/><Text style={s.applyText}>APPLY VERIFIED {selectedLabel} TO HARDWARE SCAN</Text></TouchableOpacity>:<Text style={s.warning}>RepairPilot will not use this series for exact hardware sizing. Recapture closer and repeat all three measurements.</Text>}</View>:null}
      <View style={s.actions}><TouchableOpacity disabled={!available||busy} style={[s.primary,(!available||busy)&&s.disabled]} onPress={capture}>{busy?<ActivityIndicator color="#111"/>:<MaterialCommunityIcons name="cube-scan" size={22} color="#111"/>}<Text style={s.primaryText}>{snapshot?'RECAPTURE / RESET SERIES':'CAPTURE DEPTH FRAME'}</Text></TouchableOpacity>{snapshot?<TouchableOpacity disabled={busy} style={s.secondary} onPress={resetSeries}><MaterialCommunityIcons name="restart" size={20} color="#ffb000"/><Text style={s.secondaryText}>RESET SERIES</Text></TouchableOpacity>:null}</View>
