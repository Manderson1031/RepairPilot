...
const FALLBACK_EQUIPMENT=[{id:'preview-cat',name:'CAT 320 Excavator',manufacturer:'Caterpillar',model:'320',serial:'CAT0320ABX2033',category:'Heavy equipment',notes:''}];
...
export default function TargetApp(){
 ...
 const [selectedSymptom,setSelectedSymptom]=useState(0);const [equipment,setEquipment]=useState<any[]>(FALLBACK_EQUIPMENT);const [selectedEquipment,setSelectedEquipment]=useState<any>(FALLBACK_EQUIPMENT[0]);const [repairs,setRepairs]=useState<any[]>([]);const [busy,setBusy]=useState(false);
 ...
 const loadEquipment=async(open=true)=>{try{const r=await signedFetch('/equipment');if(!r.ok)throw new Error('Could not load equipment');const list=await r.json();if(Array.isArray(list)&&list.length){setEquipment(list);const e=list[0];setSelectedEquipment({id:e.id,name:e.name,manufacturer:e.manufacturer||'',model:e.model||'',serial:e.serial||'',category:e.category||'',notes:e.notes||''})}if(open)go('equipment')}catch(e:any){Alert.alert('RepairPilot',e.message)}};
 const loadHistory=async()=>{try{const r=await signedFetch('/repairs');if(!r.ok)throw new Error('Could not load repair history');const list=await r.json();setRepairs(Array.isArray(list)?list:[]);go('history')}catch(e:any){Alert.alert('RepairPilot',e.message)}};
 const runDiagnosis=async()=>{if(busy)return;setBusy(true);try{const symptom=SYMPTOMS[selectedSymptom];const r=await signedFetch('/diagnose',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({session_id:null,equipment_profile:{id:selectedEquipment?.id||'',name:selectedEquipment?.name||'',manufacturer:selectedEquipment?.manufacturer||'',model:selectedEquipment?.model||'',serial:selectedEquipment?.serial||'',category:selectedEquipment?.category||'',notes:selectedEquipment?.notes||''},symptom,history:[],visual_evidence:[]})});const j=await r.json();if(!r.ok)throw new Error(j.detail||'Diagnosis error');Alert.alert('RepairPilot AI',j?.next_step?.question||j?.summary||'Diagnosis started successfully.',[{text:'Open scanner',onPress:openNativeScanner},{text:'OK'}])}catch(e:any){Alert.alert('RepairPilot',e.message||'Diagnosis failed')}finally{setBusy(false)}};
 const chooseEquipment=(i:number)=>{const e=equipment[i]||equipment[0]||FALLBACK_EQUIPMENT[0];setSelectedEquipment({id:e.id,name:e.name,manufacturer:e.manufacturer||'',model:e.model||'',serial:e.serial||'',category:e.category||'',notes:e.notes||''});go('diagnosis')};
 const showRepair=(i:number)=>{const r=repairs[i];if(!r)return;Alert.alert(r.equipment_name||'RepairPilot',`${r.fix||r.symptom||'Diagnostic record'}${r.status?`\n${r.status}`:''}`)};
 const maintenanceSummary=async()=>{try{await loadEquipment(false);go('maintenance')}catch{go('maintenance')}};

 const screenHotspots=useMemo(()=>{
  if(screen==='login')return <><Hotspot x={0.08} y={0.67} w={0.84} h={0.07} label="Sign in" onPress={auth}/><Hotspot x={0.08} y={0.80} w={0.84} h={0.07} label="Create account" onPress={()=>Alert.alert('RepairPilot','Account creation remains invite-based. Use your existing account to sign in.')}/></>;
  if(screen==='dashboard')return <><Hotspot x={0.06} y={0.31} w={0.27} h={0.18} label="AI diagnosis" onPress={()=>go('diagnosis')}/><Hotspot x={0.36} y={0.31} w={0.27} h={0.18} label="Hardware scanner" onPress={openNativeScanner}/><Hotspot x={0.66} y={0.31} w={0.27} h={0.18} label="AR repair assistant" onPress={openNativeAR}/><Hotspot x={0.06} y={0.50} w={0.27} h={0.18} label="My equipment" onPress={()=>loadEquipment(true)}/><Hotspot x={0.36} y={0.50} w={0.27} h={0.18} label="My diagnostics" onPress={loadHistory}/><Hotspot x={0.66} y={0.50} w={0.27} h={0.18} label="Maintenance" onPress={maintenanceSummary}/></>;
  if(screen==='equipment')return <>{[0,1,2,3].map(i=><Hotspot key={i} x={0.04} y={0.18+i*0.11} w={0.92} h={0.10} label={`Equipment ${i+1}`} onPress={()=>chooseEquipment(i)}/>)}</>;
  ...
  if(screen==='history')return <>{[0,1,2,3].map(i=><Hotspot key={i} x={0.04} y={0.18+i*0.16} w={0.92} h={0.13} label={`Repair ${i+1}`} onPress={()=>showRepair(i)}/>)}</>;
  if(screen==='maintenance')return <><Hotspot x={0.06} y={0.18} w={0.88} h={0.20} label="Equipment maintenance" onPress={()=>go('equipment')}/><Hotspot x={0.06} y={0.42} w={0.88} h={0.20} label="Diagnostic history" onPress={loadHistory}/></>;
  return null;
 },[screen,selectedSymptom,email,password,busy,token,selectedEquipment,equipment,repairs]);

 return <View style={s.page}><StatusBar style="light"/><View style={[s.stage,{width:displayW,height:displayH}]}><CropLayer screen={screen} width={displayW}/>{screenHotspots}{showNav&&NAV.map(([to,x,y,w,h])=><Hotspot key={to} x={x} y={y} w={w} h={h} label={to} onPress={()=>to==='scanner'?openNativeScanner():to==='ar'?openNativeAR():to==='equipment'?loadEquipment(true):to==='maintenance'?maintenanceSummary():go(to)}/>)}{screen==='login'&&<View pointerEvents="box-none" style={StyleSheet.absoluteFill}><TextInput value={email} onChangeText={setEmail} autoCapitalize="none" keyboardType="email-address" style={[s.invisibleInput,{left:'11%',top:'48.7%',width:'78%',height:'5.5%'}]}/><TextInput value={password} onChangeText={setPassword} secureTextEntry style={[s.invisibleInput,{left:'11%',top:'58.8%',width:'78%',height:'5.5%'}]}/></View>}{c.mode==='panel'&&<TouchableOpacity style={s.close} onPress={()=>go('dashboard')}><Text style={s.closeText}>‹ HOME</Text></TouchableOpacity>}</View></View>;
}
