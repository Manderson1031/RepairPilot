...
new_diag="<View style={s.twoButtons}><TouchableOpacity style={s.outlineHalf} onPress={takeDiagnosticPhoto}><MaterialCommunityIcons name=\"camera-outline\" size={19} color={AMBER}/><Text style={s.outlineText}>ADD PHOTO</Text></TouchableOpacity><TouchableOpacity style={s.primaryHalf} onPress={()=>setScreen('complete')}><Text style={s.primaryText}>FINISH REPAIR</Text></TouchableOpacity></View><TouchableOpacity style={s.outlineButton} onPress={()=>{const q=new URLSearchParams({mode:'guided',equipmentId:String(selected?.id||''),equipmentName:String(selected?.name||''),sessionId:String(last?.session_id||'')}).toString();Linking.openURL(`repairpilot://ar-assist?${q}`)}}><MaterialCommunityIcons name=\"cube-scan\" size={19} color={AMBER}/><Text style={s.outlineText}>OPEN GUIDED AR FOR THIS DIAGNOSIS</Text></TouchableOpacity></>}"
